...
export async function validateDDLOnly(
  ...
  try {
    ...
    const finalPackageName = packageName || vscode.workspace.getConfiguration('egovframeInitializr').get<string>('defaultPackageName', 'egovframework.example.sample')
...
export async function generatePreviews(
  ...
  const templatePromises = [
    ...
    { key: 'mapperInterface', promise: renderTemplateForPreview(templateFilePaths.mapperInterfaceTemplateFilePath, templateContext) },
    ...
    { key: 'thymeleafList', promise: renderTemplateForPreview(templateFilePaths.thymeleafListTemplateFilePath, templateContext) },
    { key: 'thymeleafRegister', promise: renderTemplateForPreview(templateFilePaths.thymeleafRegisterTemplateFilePath, templateContext) }
  ]

  // 병렬로 실행하고 결과를 객체로 변환
  const results = await Promise.all(templatePromises.map(async ({ key, promise }) => {
    try {
  ...
  const previews = results.reduce((acc, { key, result }) => {
    acc[key] = result
    return acc
